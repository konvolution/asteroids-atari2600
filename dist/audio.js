...
export class AudioEngine {
    constructor() {
        ...
        this.heartbeatTempo = 1000; // ms between beats
        this.heartbeatPhase = 0; // 0 or 1 for alternating tones
        // Active sounds
    ...
    createSquareOscillator(frequency) {
    ...
    playFire() {
        if (!this.initialized || this.muted || !this.audioContext || !this.masterGain)
            return;
        this.resume();
    ...
    calculateHeartbeatTempo(asteroidCount) {
        return 200 + (800 * Math.min(asteroidCount / 24, 1));
    }
    startHeartbeat(asteroidCount = 12) {
        if (!this.initialized)
            return;
